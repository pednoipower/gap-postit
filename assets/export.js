...
<Relationship Id="rId${sheets.length+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>
  ...
  function toXLSX(snap, cfg) {
    ...
    const causeRows = [[
      H("Ref"), H("Gap"), H("Gap label"), H("Type"), H("ใคร (actor)"), H("มักจะ (response)"), H("ตอนที่ (situation)"), H("เพราะ (reason)"),
  ...
  function toSolutionsTXT(snap, cfg) {
    ...
    L.push("");
    L.push("WHY lines: LABEL | team | role | ใคร (actor) | มักจะ (response) | ตอนที่ (situation) | เพราะ (reason)");
    ...
    for (const g of (snap.groups || [])) {
      ...
      const sent = x => `ใคร ${clean(x.actor)} | มักจะ ${clean(x.body)} | ตอนที่ ${clean(x.context)} | เพราะ ${clean(x.reason)}`;
